...
proc adi_xcvr_parameters {file_paths parameters} {
    ...
    #puts $file_content
    set match ""
    if {[regexp {QPLL[0-9]+} $cfng_file_path match]} {
        puts "Valoarea extrasă este: $match"
    } else {
        puts "Nu s-a găsit nicio potrivire!"
    }
    ...
    for {set i 0} {$i < [llength $matches]} {incr i 3} {
        ...
        if {[dict exists $correction_map $param]} {
         ...
         if {[string first $match $param] == 0} {
            # Elimină cifra din prefix
            if {[regexp {^(QPLL)[0-9]+(.*)} $param _ prefix rest]} {
                set corr_param "${prefix}${rest}"
                puts "Parametrul modificat: $param -> $corr_param"
            }
        }
