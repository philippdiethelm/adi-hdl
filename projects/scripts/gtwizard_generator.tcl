...
proc get_diff_params { {lane_rate_l {}} {pll_type {}}  {ref_clk_l {}} {keep_ip "true"} } {
  ...
  puts "Path-ul cu două foldere înapoi: $hdl_projects_path"

  ## Generate configurations
  ad_gth_generator $lane_rate_l $pll_type $ref_clk_l

  puts "current_dir inainte de a executa parser: $current_dir"
  puts "project name inainte de a executa parser: $project_name"
  ...
  if {[file tail $dst_path] eq "ip"} {
    if { [catch { exec $hdl_projects_path/scripts/gtwiz_parser.pl $gt_type } e] } {
      puts "path cand s-a incercat executia parser: $[pwd]"
      cd $current_dir
